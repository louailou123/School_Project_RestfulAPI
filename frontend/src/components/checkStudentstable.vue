        ...
        <DataTable v-model:filters="filters" :value="students" paginator :rows="8" dataKey="id" filterDisplay="row" :loading="loading"
         ...
         <Column header="Address" filterField="Address" style="min-width: 12rem">
            ...
            <Column style="min-width: 12rem">
                     <template #body="{ data }">
                    <div class="flex items-center gap-2">
                        <RouterLink :to="`/student/update/${data.id}`" ><Button style="width: 60px;" rounded severity="warn">Edit</Button></RouterLink> 
                        <Button type="submit" @submit="deleteStudent(data.id)" rounded severity="warn" style="width: 60px;">Delete</Button>
                    </div>
                </template>
